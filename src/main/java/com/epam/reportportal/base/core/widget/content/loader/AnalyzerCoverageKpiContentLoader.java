...
@Service
public class AnalyzerCoverageKpiContentLoader implements LoadContentStrategy {

  private final AnalyzerProjectMetricsProvider analyzerProjectMetricsProvider;

  public AnalyzerCoverageKpiContentLoader(
  ...
  public Map<String, ?> loadContent(List<String> contentFields, Map<Filter, Sort> filterSortMapping,
    ...
    AnalyzerCoverageRs response = analyzerProjectMetricsProvider.getProjectCoverage(
        extractProjectId(filter));

    Map<String, Object> content = new LinkedHashMap<>();
    content.put("currentSprint", response.getCurrentSprint());
    content.put("previousSprint", response.getPreviousSprint());
    content.put("trend", response.getTrend());
    content.put("missingSprintAttribute", response.isMissingSprintAttribute());
    return content;
  }
  ...
  private Long extractProjectId(Filter filter) {
    return filter.getFilterConditions().stream()
        ...
        .map(Long::valueOf)
        .findFirst()
        .orElseThrow(() -> new IllegalStateException("Project scope is required for analyzer coverage widget"));
  }
}